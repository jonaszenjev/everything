...
public class MCreatorKey1 extends Elementsadvancedfun.ModElement {
	private KeyBinding keys;

	public MCreatorKey1(Elementsadvancedfun instance) {
		super(instance, 6);
	}
	...
	public void init(FMLInitializationEvent event) {
		keys = new KeyBinding("key.mcreator.key1", Keyboard.KEY_K, "key.categories.misc");
	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (!FMLClientHandler.instance().isGUIOpen(GuiChat.class)) {
			if (org.lwjgl.input.Keyboard.isKeyDown(keys.getKeyCode())) {
				advancedfun.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage());
	...
	private static void pressAction(EntityPlayer entity) {
			...
			return;
		{
			...
			MCreatorCmenuCommandExecuted.executeProcedure($_dependencies);
		}
	}
}
